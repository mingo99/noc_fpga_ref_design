...
# ---------------------------------------------------------------------
set TOP_LEVEL_MODULE    tb_mlp_conv2d

# ---------------------------------------------------------------------
# Define the simulation flow
# ---------------------------------------------------------------------
# Supported options are STANDALONE, FULLCHIP_BFM
set FLOW                 "STANDALONE"
...
# ---------------------------------------------------------------------
# set ::env(ACE_INSTALL_DIR)/opt/achronix/ace/8.2.1/Achronix-linux/
...
switch $FLOW {
    "FULLCHIP_BFM" {
        set argv "$SIM_FILELIST ../../src/filelist_b60.tcl ../../scripts/sim_template_bfm.f"
    }
    "FULLCHIP_RTL" {
        set argv "$SIM_FILELIST ../../src/filelist_b60.tcl ../../scripts/sim_template_bfm.f"
    }
    "STANDALONE" {
        set argv "$SIM_FILELIST ../../src/filelist_b60.tcl ../../scripts/sim_template.f"
    }
...
switch $FLOW {
    ...
    "STANDALONE" {
        ...
        quietly append QUESTA_OPTIONS    " +define+ACX_SIM_STANDALONE_MODE +define+AXI_MEM_CHECK_AXI4_PORT"
    }
    default { puts "Error : Unrecognised FLOW value of $FLOW"; return -1 }
}
